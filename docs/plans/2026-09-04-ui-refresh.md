...
- Game rules, schedule, scoring, sync/conflict, RLS, and deletion behavior are
  frozen and out of scope. This plan changes presentation only.
...
- `ios/GridRaceTests/GameRulesTests.swift` and
  ...
  avatar-palette, and existing-validator proof only).
- Presentation-only state in views (e.g. `@AccessibilityFocusState`,
  `@Environment` adaptations). No model, persistence, sync, or rule changes
  unless a named unit explicitly justifies it.
...
`AccountView.swift` + `AccountTests.swift`) and may run concurrently. U-04
...
## U-05 Account + avatar (Worker D — `AccountView.swift` + `GridRaceTests/AccountTests.swift` only)

- Keep all auth/sync semantics; presentation only. Add the exact reassurance
  line `Your email is never shown to other players.` (contract-backed by
...
- State-to-focus map (covers the full required order; one speech owner per
  ...
  - animated reveal rows → after each complete five-tile row becomes visible,
    programmatically focus that row; after the final row, focus the summary;
  - Reduce Motion reveal → present the full state immediately, focus the
    answer first, and preserve manual traversal order answer → viewer board →
    stable roster boards → top-to-bottom rows → summary without
    auto-cycling focus through already-visible rows;
...
- Record per-screen evidence rows: device + OS + orientation + content-size →
  input → expected → observed. Capture a VoiceOver transcript for invalid
  input, solve/fail, animated reveal, and Reduce Motion reveal; each message
  occurs once and in the required order (countdown → draft/error → terminal
  → answer → rows → summary).
...
- U-06 starts after Wave 1 + U-04 merge; R-01 after U-06; no second reviewer
  loop (accepted fixes get targeted controller inspection + rerun, per
  runbook).

# Risk tier and verification matrix

- Tier: Medium (focused UI behavior, no rule/auth/schema/contract change; no
  model, `.pbxproj`, or new-transition work remains after adjudication).
...
- Required gates (runbook canon; rediscover the simulator `id=` via `-list` /
  ...
  - Focused additions: avatar-palette contrast test (U-05), duplicate-vector
    feedback assertion (U-03), existing profile-validator tests plus manual
    Save-state checks (U-05); rendered tile labels remain in manual VoiceOver
  ...
  - Manual evidence matrix (each row records device + OS + orientation +
    ...
    Motion on/off; VoiceOver transcripts (invalid, solve/fail, animated +
    nonanimated reveal); grayscale tile/key check; provisioned-SE portrait
...
| RR-01 | High | U-06 reveal focus | Revised map preserved traversal order but did not move focus through staged rows/summary | Plan U-06 vs `screen-flow.md:169-170` | accepted | Animated reveal focuses each completed row then summary; Reduce Motion focuses answer first and preserves manual semantic order | VoiceOver transcripts show the required focus/traversal sequence without duplicate speech |
...
- *Any focus change causing dual speech via focus + `UIAccessibility.post`
  (UI-RT-05).
- *Any avatar seed→symbol remap or rewrite of the tutorial disclaimer, reveal
  summary, Hard Mode footer, or duplicate-letter explanation (UI-RT-12/13).
