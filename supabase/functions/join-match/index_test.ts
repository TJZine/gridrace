...
function post(body: unknown, token: string | null = TOKEN): Request {
  ...
  return new Request("http://localhost/join-match", {
    method: "POST",
    headers,
...
function validBody(): JsonObject {
  return { client_build: 7, join_code: "abc234" };
}
...
Deno.test("joins a match through the join_match RPC", async () => {
  ...
  assertEquals(invoked.rpcCalls[0].name, "join_match");
...
const failures: FailureCase[] = [
  ...
  {
    name: "rejects a body missing join_code",
  ...
  },
  {
    ...
    request: () => post({ ...validBody(), match_id: MATCH_ID }),
    status: 400,
    code: "internal_error",
    rpcCalls: 0,
    authenticateCalls: 0,
  ...
  {
    name: "maps a typed room-full error",
    request: () => post(validBody()),
    rpcResults: [domainError("room_full")],
    status: 409,
    code: "room_full",
    rpcCalls: 1,
    authenticateCalls: 1,
  ...
  },
  {
    ...
    request: () => post(validBody()),
    rpcResults: [{ data: { member: {} }, error: null }],
    status: 500,
    code: "internal_error",
    rpcCalls: 1,
    authenticateCalls: 1,
