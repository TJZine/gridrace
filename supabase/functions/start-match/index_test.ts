...
function post(body: unknown, token: string | null = TOKEN): Request {
  ...
  return new Request("http://localhost/start-match", {
    method: "POST",
    headers,
...
Deno.test("starts a match through the start_match RPC", async () => {
  ...
  assertEquals(invoked.rpcCalls[0].name, "start_match");
...
const failures: FailureCase[] = [
  ...
  {
    name: "maps a typed not-host error",
    request: () => post(validBody()),
    rpcResults: [domainError("not_host")],
    status: 403,
    code: "not_host",
    rpcCalls: 1,
    authenticateCalls: 1,
  ...
  },
  {
    ...
    request: () => post(validBody()),
    rpcResults: [{ data: { started: true }, error: null }],
    status: 500,
    code: "internal_error",
    rpcCalls: 1,
    authenticateCalls: 1,
